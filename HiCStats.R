...
hicProDir <- strsplit(hicProDir, ",")[[1]]
samples <- strsplit(samples, ",")[[1]]
allDirs <- list.files( file.path( hicProDir, "hic_results/data"), full.names=TRUE)
if( !identical( samples, "all") ){
    outDirs <- allDirs[basename(allDirs) %in% samples]
}else{
    outDirs <- allDirs
}

statFiles <- list.files( outDirs, pattern="mRSstat", full.names=TRUE )
names( statFiles ) <- gsub( ".mRSstat", "", basename(statFiles) )
mergeFiles <- list.files( outDirs, pattern="_allValidPairs.mergestat", full.names=TRUE )
...
allMergeData <- melt( lapply( mergeFiles, read.table),
                     id.vars="V1",
...
allStatData <- melt( lapply( statFiles, read.table ),
